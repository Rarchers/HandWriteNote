...
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    out = tf.add(tf.matmul(dense, w_out), b_out, name="output")
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            if step % 100 == 0 and step != 0:
                ...
                acc, predicts, max_idx_pre, max_idx_le = sess.run([accuracy ,output,max_idx_p,max_idx_l],
                               feed_dict={X: batch_x_test, Y: batch_y_test, keep_prob: 1., train_phase: False})
                print("第%s步，训练准确率为：%s" % (step, acc))
                print("第%s步，max_idx_p：%s" % (step, max_idx_pre))
                print("第%s步，max_idx_l：%s" % (step, max_idx_le))
                # 如果准确率大90%,保存模型,完成训练
                if acc > 0.9:
                    save_path = saver.save(sess, "model1\\crack_capcha.model", global_step=step)
                    print(save_path)
                    break
            step += 1
